...
from ekonlpy.data.tagset import aux_tags
import nltk
...
class MPCK(object):
    ...
    def train_classifier(self, dataset, feature_fn_name='word', train_ratio=0.8, verbose=False,
        ...
        if feature_fn_name in ['best_word', 'best_bigram', 'best_trigram']:
            ...
            for word, freq in word_fd.items():
                pos_score = BigramAssocMeasures.chi_sq(label_word_fd[self._positive_label][word],
                                                       (freq, pos_word_count), total_word_count)
                neg_score = BigramAssocMeasures.chi_sq(label_word_fd[self._negative_label][word],
                                                       (freq, neg_word_count), total_word_count)
                word_scores[word] = pos_score + neg_score

            best_cnt = int(len(word_scores) * best_ratio)
            best = sorted(word_scores.items(), key=lambda item: item[1], reverse=True)[:best_cnt]
        ...
        if verbose:
            print('No. of samples: {}, Pos: {}, Neg: {}'.format(len(feats), len(posfeats), len(negfeats)))
    ...
    def evaluate_confusion_matrix(self, actual, predicted, actual_pos_val=1, actual_neg_val=-1, verbose=False):
        return evaluate_confusion_matrix(actual, predicted,
                                         actual_pos_val=actual_pos_val,
                                         actual_neg_val=actual_neg_val,
                                         verbose=verbose)


def evaluate_confusion_matrix(actual, predicted, actual_pos_val=1, actual_neg_val=-1, verbose=False):
    t_pos = 0
    f_pos = 0
    t_neg = 0
    f_neg = 0
    ...
    metrics = {'Pearson corr': pr[0], 'Spearman corr': sr[0], 'Accuracy': all_acc,
               'Pos precision': pos_acc, 'Pos recall': pos_recall,
               'Neg precision': neg_acc, 'Neg recall': neg_recall}
    if verbose:
        print(metrics)
    return metrics
